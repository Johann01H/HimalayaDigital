...
<table class="table data-table stripe hover nowrap">
    ...
    <tbody>
        ...
        <tr class="text-center">
            <td class="table-plus">{{ $contenido->ots->referencia }}</td>
            <td>{{$contenido->ots->clientes->nombre }}</td>
            <td>{{ $contenido->ots->nombre }}</td>
            <td>Alto</td>
            <td>{{ $contenido->created_at }}</td>
            <td>{{ $contenido->fecha_entrega_cuentas }}</td>
            <td>{{ $contenido->fecha_entrega_cliente }}</td>
            <td>
                @switch($contenido->estados->id)
                @case(1)
                <span class="badge badge-success"> {{ $contenido->estados->nombre }}</span>
                @break
                @case(2)
                <span class="badge badge-primary"> {{ $contenido->estados->nombre }}</span>
                @break
                @case(3)
                <span class="badge badge-info"> {{ $contenido->estados->nombre }}</span>
                @break
                @case(4)
                <span class="badge badge-warning"> {{ $contenido->estados->nombre }}</span>
                @break
                @case(5)
                <span class="badge badge-warning"> {{ $contenido->estados->nombre }}</span>
                @break
            ...
            <td>{{ $contenido->users->nombre ?? 'No se encontro el usuario relacionado'}}</td>
            <td>
