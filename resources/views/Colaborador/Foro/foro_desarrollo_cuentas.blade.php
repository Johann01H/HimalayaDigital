    ...
    <table class="table data-table stripe hover nowrap">
        ...
        <tbody>
            ...
            <tr class="text-center">
                <td class="table-plus">{{ $cuenta->ots->referencia }}</td>
                <td>{{$cuenta->ots->clientes->nombre }}</td>
                <td>{{ $cuenta->ots->nombre }}</td>
                <td>Alto</td>
                <td>{{ $cuenta->created_at }}</td>
                <td>{{ $cuenta->fecha_entrega_cuentas }}</td>
                <td>{{ $cuenta->fecha_entrega_cliente }}</td>
                <td>
                    @switch($cuenta->estados->id)
                    @case(1)
                    <span class="badge badge-success"> {{ $cuenta->estados->nombre }}</span>
                    @break
                    @case(2)
                    <span class="badge badge-primary"> {{ $cuenta->estados->nombre }}</span>
                    @break
                    @case(3)
                    <span class="badge badge-info"> {{ $cuenta->estados->nombre }}</span>
                    @break
                    @case(4)
                    <span class="badge badge-warning"> {{ $cuenta->estados->nombre }}</span>
                    @break
                    @case(5)
                    <span class="badge badge-warning"> {{ $cuenta->estados->nombre }}</span>
                    @break
                ...
                <td>{{ $cuenta->users->nombre ?? 'No se encontro el usuario relacionado'}}</td>
                <td>
