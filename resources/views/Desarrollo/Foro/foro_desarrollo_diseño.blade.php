...
<table class="table data-table stripe hover">
    ...
    <tbody>
        ...
        <tr class="text-center">
            <td class="table-plus">{{ $diseño->ots->referencia }}</td>
            <td>{{$diseño->ots->clientes->nombre }}</td>
            <td>{{ $diseño->ots->nombre }}</td>
            <td>Alto</td>
            <td>{{ $diseño->created_at }}</td>
            <td>{{ $diseño->fecha_entrega_cuentas }}</td>
            <td>{{ $diseño->fecha_entrega_cliente }}</td>
            <td>
                @switch($diseño->estados->id)
                @case(1)
                <span class="badge badge-success"> {{ $diseño->estados->nombre }}</span>
                @break
                @case(2)
                <span class="badge badge-primary"> {{ $diseño->estados->nombre }}</span>
                @break
                @case(3)
                <span class="badge badge-info"> {{ $diseño->estados->nombre }}</span>
                @break
                @case(4)
                <span class="badge badge-warning"> {{ $diseño->estados->nombre }}</span>
                @break
                @case(5)
                <span class="badge badge-warning"> {{ $diseño->estados->nombre }}</span>
                @break
            ...
            <td>{{ $diseño->users->nombre ?? 'No se encontro el usuario relacionado'}}</td>
            <td>
